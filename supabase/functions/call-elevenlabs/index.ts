...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Create webhook URL for AI conversation with ElevenLabs
    const webhookUrl = `${appUrl}/functions/v1/twilio-webhook-elevenlabs`;
    ...
    console.log('Making Twilio API call with ElevenLabs:', {
      from: fromNumber,
      to: lead.phone,
      twilioUrl: twilioUrl,
    ...
    const callResponse = await fetch(twilioUrl, {
      ...
      body: new URLSearchParams({
        ...
        Url: webhookUrl,
        Method: 'POST',
        StatusCallback: `${appUrl}/functions/v1/twilio-webhook-elevenlabs`,
    ...
    // Update lead status to 'contacted' after successful call
    ...
    return new Response(
      JSON.stringify({ 
        success: true,
        message: `ElevenLabs AI call initiated to ${lead.name} at ${lead.phone}`,
