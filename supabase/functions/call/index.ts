...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Create webhook URL for AI conversation
    const webhookUrl = `${appUrl}/functions/v1/twilio-webhook`;
    ...
    const callResponse = await fetch(twilioUrl, {
      ...
      body: new URLSearchParams({
        ...
        Url: webhookUrl,
        Method: 'POST',
        StatusCallback: `${appUrl}/functions/v1/twilio-webhook`,
    ...
    return new Response(
      JSON.stringify({ 
        success: true,
        message: `Call initiated to ${lead.name} at ${lead.phone}`,
