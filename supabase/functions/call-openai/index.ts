...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Create webhook URL for AI conversation with OpenAI TTS
    const webhookUrl = `${appUrl}/functions/v1/twilio-webhook-openai`;
    ...
    console.log('Making Twilio API call with OpenAI TTS:', {
      from: fromNumber,
      to: lead.phone,
      twilioUrl: twilioUrl,
    ...
    const callResponse = await fetch(twilioUrl, {
      ...
      body: new URLSearchParams({
        ...
        Url: webhookUrl,
        Method: 'POST',
        StatusCallback: `${appUrl}/functions/v1/twilio-webhook-openai`,
    ...
    return new Response(
      JSON.stringify({ 
        success: true,
        message: `OpenAI TTS call initiated to ${lead.name} at ${lead.phone}`,
